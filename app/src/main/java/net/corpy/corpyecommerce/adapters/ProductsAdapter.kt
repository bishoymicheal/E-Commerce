    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        EcommerceRepo.loadProductCategory(product.dep_id.toString()) { status, productCat ->
            if (App.getCurrentLanguageKey(holder.itemView.context) == "ar") {
                holder.departmentName.text = productCat?.ar_name ?: product.ar_title
            } else {
                holder.departmentName.text = productCat?.en_name ?: product.en_title
            }
        }
        if (App.getCurrentLanguageKey(holder.itemView.context) == "ar") {
